...
> 最后更新：2026-06-30（写作 MVP 卡片式 Step Flow 落地）
...
| M10-lite | 写作搭建训练 MVP | ✅ 已增强 | 小作文/大作文分流；已落地 `选方向 -> 高质量半填空框架 -> 局部自由写作 -> AI 批改沉淀` 的卡片式 Step Flow |
| M14-lite | 翻译专项 MVP | ✅ 首版已落地 | 按 `找主干 -> 拆修饰 -> 整句翻译` 三步训练，AI 分步纠偏并沉淀固定搭配 |
| M15-lite | 完形填空 MVP | ✅ 首版已落地 | 单空选择 + 用户说理由 + AI 判断依据类型 + 第二轮给答案和错因 |
...
- 首页不再重复放置 `误解本` 和 `高级智能英语教练系统` 入口；这两个能力已经是底部 tab
- `最近复盘` 从首页移出，后续归入 `高级智能英语教练系统`，避免首页过重
...
- 专项训练 MVP 已新增统一 practice 接口：
  - `GET /api/v1/practice/writing/task?userId=&type=small|large`
  - `POST /api/v1/practice/writing/coach`
  - `GET /api/v1/practice/translation/task?userId=`
  - `POST /api/v1/practice/translation/coach`
  - `GET /api/v1/practice/cloze/task?userId=`
  - `POST /api/v1/practice/cloze/coach`
- 写作专项 MVP 当前目标已调整：
  - 写作二级页将 `小作文 / 大作文` 分流到写作搭建训练
  - 前端已改成卡片式 Step Flow，同页分步但不跳页，原则是 `内容不减，暴露量变小`
  - 第一步仍保留 `开头起笔 / 中间展开 / 结尾收束` 的方向选择，但现在每次只展示一个问题，训练用户写作前的思考动作
  - AI 反馈必须先分析用户选择的方向是否有偏差，再判断用户补充内容属于 `真实写作思路` 还是 `求助/焦虑/发牢骚`
  - 如果用户表达的是不会写、词汇量不够、不会分段等困难，AI 应先降低任务难度并给出可执行下一步，而不是假装用户已经完成作文思路
  - 第二步进入 `高质量半填空作文框架训练`：系统给出考研范文水准的骨架句式，不允许变成小学生/初中生表达
  - 框架填空也采用逐空展示，每次只暴露一个空白；空白处不是填词游戏，用户可以自由写短语、一个句子，甚至 1-3 句
  - 第三步 AI 针对用户填入内容做批改：贴题度、逻辑、语法、表达升级、可沉淀短语/句型
  - 反馈中会给出可沉淀到误解本的 `写作表达 / 固定搭配`
- 翻译专项 MVP：
  - 用户按三步提交：`主干判断 / 修饰成分 / 整句译文`
  - AI 分步指出最关键问题，不一上来给标准译文
  - 第三步后展示参考译文与错因，并提供固定搭配沉淀
- 完形填空 MVP：
  - 当前采用单空训练，降低进入压力
  - 用户先选 A/B/C/D，再写选择理由
  - AI 判断理由属于语义、搭配、逻辑还是猜测，并继续追问
  - 第二轮后展示答案、错因和可沉淀的固定搭配/易混词

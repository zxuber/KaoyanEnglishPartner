...
> 最后更新：2026-06-23（首页数据闭环已增强，阅读 MVP 已切到更接近真题的教练式交互）
> 当前阶段：第一期 MVP 开发中（M2/M4/M5 已落地首版，继续 LAN 联调与功能开发）
...
| M4 | 今日训练首页骨架 | ✅ 首版已落地 | 首页已接入后端聚合数据，首屏展示继续上次训练、今日训练、专项入口、最近复盘；返回首页会自动刷新 |
...
- 阅读模块首版已接入：
  - `GET /api/v1/reading/article`
  - `POST /api/v1/reading/coach`
  - 先固定一篇文章和两道题，验证“文章 -> 说思路 -> AI 追问 -> 给答案”闭环

...
## 15. 阅读原文内做可点击交互时，微信小程序会出现不换行 / 右侧被裁
...
阅读页为了支持“直接在原文里点词/点句”，一度使用了多层 `text` 组件嵌套来承载：
...
微信小程序对多层 `text` 嵌套下的复杂换行、点击和高亮组合支持不稳定。
...
后续如果还要在微信小程序里做“原文内可点击”的富文本交互：

- 不要优先走多层 `text` 嵌套方案
- 优先使用块级容器 + 流式布局
- 先保证换行和段落结构稳定，再叠加高亮和手势
...
## 15. DeepSeek Key 明明写了，但后端仍然返回 401
...
阅读短句翻译、阅读 AI 追问或学习方案生成出现：

- `Translate fallback triggered: DeepSeek API error 401`
- 或其他 `DeepSeek API error 401`

同时你已经确认：

- `docs/API_KEYS.md` 里记录了真实 key
- `~/.zshrc` 或 `~/.zprofile` 里也已经写了 `DEEPSEEK_API_KEY`

### 根因

问题不一定是 key 无效，也可能是：

- 当前跑着的后端 Java 进程没有继承 `DEEPSEEK_API_KEY`

这在 `Trae / IDEA / VS Code Java Run` 这类 GUI 启动方式下很常见，因为它们未必会读取你的 shell 初始化文件。
...
那就说明这个后端进程虽然启动了，但实际上没有拿到 LLM key。
...
```bash
zsh -lc 'source ~/.zprofile >/dev/null 2>&1; cd /你的项目/backend && mvn spring-boot:run'
```

方案二：GUI 启动时，手动把以下环境变量配进运行配置：

- `DEEPSEEK_API_KEY`

### 结论

- 单词翻译有时还能成功，是因为它可能命中本地词典或项目词库
- 短句翻译一定走 LLM，所以只要后端进程没带上 `DEEPSEEK_API_KEY`，就一定会 401
